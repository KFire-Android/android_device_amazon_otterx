...
#define CARD_OTTER_DEFAULT CARD_OMAP4_ABE
...
struct otter_audio_device {
    ...
    int in_call;
    float voice_volume;
    struct otter_stream_in *active_input;
    struct otter_stream_out *active_output;
    bool mic_mute;
    int tty_mode;
...
static void select_output_device(struct otter_audio_device *adev);
static void select_input_device(struct otter_audio_device *adev);
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume);
static int do_input_standby(struct otter_stream_in *in);
static int do_output_standby(struct otter_stream_out *out);
...
static int start_call(struct otter_audio_device *adev)
{
    LOGE("Opening modem PCMs -- NOT SUPPORTED");

    return -ENOMEM;
}

static void end_call(struct otter_audio_device *adev)
{
    LOGE("Closing modem PCMs -- NOT SUPPORTED");
}

static void set_eq_filter(struct otter_audio_device *adev)
{
...
{
    struct otter_audio_device *adev = (struct otter_audio_device *)data;
...
static void set_incall_device(struct otter_audio_device *adev)
{
	LOGW("set_incall_device");
/*
    int device_type;
...
static void set_input_volumes(struct otter_audio_device *adev, int main_mic_on,
                              int headset_mic_on, int sub_mic_on)
{
    ...
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        int sub_mic_volume = VOICE_CALL_SUB_MIC_VOLUME_MAGURO;
...
static void set_output_volumes(struct otter_audio_device *adev, bool tty_volume)
{
#if 0
    unsigned int channel;
    int speaker_volume;
    ...
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        /* Voice call */
        speaker_volume = VOICE_CALL_SPEAKER_VOLUME_MAGURO;
        headset_volume = VOICE_CALL_HEADSET_VOLUME_MAGURO;
        earpiece_volume = VOICE_CALL_EARPIECE_VOLUME_MAGURO;
        mixer_ctl_overrange = adev->mixer_ctls.vx_dl2_volume;
    } else if (adev->mode == AUDIO_MODE_IN_COMMUNICATION) {
        /* VoIP */
        speaker_volume = VOIP_SPEAKER_VOLUME_MAGURO;
        headset_volume = VOIP_HEADSET_VOLUME_MAGURO;
        earpiece_volume = VOIP_EARPIECE_VOLUME_MAGURO;
    } else {
        /* Media */
        speaker_volume = NORMAL_SPEAKER_VOLUME_MAGURO;
        if (headphone_on)
            headset_volume = NORMAL_HEADPHONE_VOLUME_MAGURO;
        else
            headset_volume = NORMAL_HEADSET_VOLUME_MAGURO;

        earpiece_volume =  NORMAL_EARPIECE_VOLUME_MAGURO;
    }
...
static void force_all_standby(struct otter_audio_device *adev)
{
    struct otter_stream_in *in;
    struct otter_stream_out *out;
...
static void select_mode(struct otter_audio_device *adev)
{
    if (adev->mode == AUDIO_MODE_IN_CALL) {
...
static int start_output_stream(struct otter_stream_out *out)
{
#if DEBUG
	LOGW("%s",__FUNCTION__);
#endif
    struct otter_audio_device *adev = out->dev;
    unsigned int card = CARD_OTTER_DEFAULT;
...
*/
    ...
    out->config.start_threshold = SHORT_PERIOD_SIZE * 1;
    out->config.avail_min = LONG_PERIOD_SIZE;
    out->low_power = 0;

    //out->pcm = pcm_open(card, port, PCM_OUT | PCM_MMAP | PCM_NOIRQ, &out->config);
	out->pcm = pcm_open(card, port, PCM_OUT | PCM_MMAP, &out->config);
	LOGW("channels %u, rate %u, period_size %u, period_count %u, format %d start_threshold %u stop_threshold %u silence_threshold %u avail_min %d", 
		out->config.channels, out->config.rate, out->config.period_size, out->config.period_count, 
		out->config.format, out->config.start_threshold, out->config.stop_threshold, out->config.silence_threshold,
...
static void add_echo_reference(struct otter_stream_out *out,
...
static void remove_echo_reference(struct otter_stream_out *out,
                                  struct echo_reference_itfe *reference)
{
    pthread_mutex_lock(&out->lock);
...
static void put_echo_reference(struct otter_audio_device *adev,
                          struct echo_reference_itfe *reference)
{
    if (adev->echo_reference != NULL &&
...
static struct echo_reference_itfe *get_echo_reference(struct otter_audio_device *adev,
...
static int get_playback_delay(struct otter_stream_out *out,
                       size_t frames,
...
{
    struct otter_stream_out *out = (struct otter_stream_out *)stream;
...
static int start_input_stream(struct otter_stream_in *in)
{
#if DEBUG
	LOGW("%s",__FUNCTION__);
#endif
    ...
    int ret = 0;
    struct otter_audio_device *adev = in->dev;
...
{
    struct otter_stream_in *in = (struct otter_stream_in *)stream;
...
static void get_capture_delay(struct otter_stream_in *in,
                       size_t frames,
                       struct echo_reference_buffer *buffer)
{
...
static int32_t update_echo_reference(struct otter_stream_in *in, size_t frames)
{
    struct echo_reference_buffer b;
    b.delay_ns = 0;
...
static void push_echo_reference(struct otter_stream_in *in, size_t frames)
{
                                   ...
                                   struct resampler_buffer* buffer)
{
    ...
    in = (struct otter_stream_in *)((char *)buffer_provider -
                                   offsetof(struct otter_stream_in, buf_provider));
...
static ssize_t read_frames(struct otter_stream_in *in, void *buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
static ssize_t process_frames(struct otter_stream_in *in, void* buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
                                   ...
                                   struct audio_stream_out **stream_out)
{
    ...
    struct otter_audio_device *ladev = (struct otter_audio_device *)dev;
    struct otter_stream_out *out;
    int ret;

    out = (struct otter_stream_out *)calloc(1, sizeof(struct otter_stream_out));
    if (!out)
        return -ENOMEM;
...
{
    struct otter_audio_device *adev = (struct otter_audio_device *)dev;
    struct str_parms *parms;
    char *str;
    char value[32];
    int ret;
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    in = (struct otter_stream_in *)calloc(1, sizeof(struct otter_stream_in));
    if (!in)
        return -ENOMEM;
...
static int adev_close(hw_device_t *device)
{
	...
	LOGW("%s",__FUNCTION__);
#endif
    struct otter_audio_device *adev = (struct otter_audio_device *)device;
                     ...
                     hw_device_t** device)
{
    ...
    struct otter_audio_device *adev;
    int ret;

    if (strcmp(name, AUDIO_HARDWARE_INTERFACE) != 0)
        return -EINVAL;

    adev = calloc(1, sizeof(struct otter_audio_device));
    if (!adev)
        return -ENOMEM;
